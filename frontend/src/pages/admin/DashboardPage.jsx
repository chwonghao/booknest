...
setData({ summaryData: mockSummaryData, salesData: mockSalesData, recentOrders: recentOrders, lowStockProducts: lowStockProducts, orderStatusData: orderStatusData });
        ...
        setError("Không thể tải dữ liệu cho bảng điều khiển. Vui lòng thử lại.");
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={8}>
          <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column', height: 400 }}>
            <Typography variant="h6" gutterBottom>Doanh thu tuần</Typography>
            {loading ? (
        ...
        <Grid item xs={12} md={4}>
          <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column', height: 400 }}>
            <Typography variant="h6" gutterBottom>Tỷ lệ trạng thái đơn hàng</Typography>
                  ...
                  data.lowStockProducts.map(product => (
                    <ListItem key={product.id} divider>
                      <ListItemText
                        ...
                        secondary={`Chỉ còn: ${product.stockQuantity} sản phẩm`}
                      />
                    </ListItem>
                  ))
                ) : <Typography sx={{ p: 2, color: 'text.secondary' }}>Không có sản phẩm nào sắp hết hàng.</Typography>}
              </List>
            )}
          </Paper>
        </Grid>
          ...
          <Paper sx={{ p: 2, height: '100%' }}>
            ...
            <List dense sx={{ maxHeight: 300, overflow: 'auto' }}>
              {data?.recentOrders && data.recentOrders.length > 0 ? (
                data.recentOrders.map((order, index) => (
                  <React.Fragment key={order.id}>
                    <ListItem alignItems="flex-start">
                      <ListItemText
                        primary={`Đơn #${order.id} - ${order.userName}`}
                ...
                <Typography sx={{ p: 2, color: 'text.secondary' }}>Không có đơn hàng nào gần đây.</Typography>
              )}
            </List>
            )}
          </Paper>
