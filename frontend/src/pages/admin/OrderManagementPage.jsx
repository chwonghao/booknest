...
export default function OrderManagementPage() {
  ...
  const headCells = [
    ...
    { id: 'totalAmount', numeric: true, disablePadding: false, label: 'Total' },
  ...
  const handleRowClick = async (order) => {
    ...
    try {
      ...
      setNotification({ open: true, message: 'Could not load product details.', severity: 'error' });
    }
  };
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      setNotification({ open: true, message: 'Order status updated successfully!', severity: 'success' });
      handleCloseDetail();
    } catch (err) {
      console.error("Failed to update status:", err);
      setNotification({ open: true, message: 'Failed to update order status.', severity: 'error' });
    } finally {
      setIsUpdating(false);
    }
  };
        ...
        {selectedOrder && (
          <DialogContent dividers>
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2 }}>
              ...
              <Typography><strong>Customer:</strong> {selectedOrder.userName || `User ID: ${selectedOrder.userId}`}</Typography>
